...
- **Compile a book to PDF.** The book navigator's new share menu renders
  ...
  "&lt;Book name&gt;.pdf".
- **PDF Layout setting.** Choose how PDFs are built, right in the share
  menu: "One long page" (the continuous, nothing-sliced page — still the
  default) or "A4 pages" — real A4 pagination with line-aware page breaks,
  produced by the same engine as printing, honouring `\newpage`.
